...
from pywaybackup.SnapshotCollection import Snapshot
...
class Message(Worker):
    ...
    def store(self, verbose: bool = None, result: str = "", info: str = "", content: str = ""):
        ...
        if verbose is False or verbose is None:
            ...
            self.message = {
                "verbose": False,
                "content": f"{self.worker.snapshot.counter}/TOTAL_PLACEHOLDER - W:{self.worker.id} - {result}{content}{self.worker.snapshot.timestamp} - {self.worker.snapshot.url_origin}",
            }
            self.buffer.append(self.message)
