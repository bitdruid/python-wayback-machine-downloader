...
from pywaybackup.SnapshotCollection import Snapshot, SnapshotCollection
...
class Downloader:
    def __init__(self, output: str, retry: int, no_redirect: bool, delay: int, workers: int, SnapshotCollection: SnapshotCollection):
        self.output = output
        self.retry = retry
            ...
            while True:
                ...
                while worker.attempt <= retry_max_attempt:  # retry as given by user
                    worker.message.store(
                        verbose=True,
                        content=f"\n-----> Worker: {worker.id} - Attempt: [{worker.attempt}/{retry_max_attempt}] Snapshot ID: [{worker.counter}/{self.sc._snapshot_total}]",
                    )
                    download_attempt = 1
                    ...
                    while download_attempt <= download_max_attempt:  # reconnect as given by system
                        ...
                        try:
                            download_status = download(self.output, self.no_redirect, worker=worker)
                            ...
                            if isinstance(e, (timeout, ConnectionRefusedError, ConnectionResetError)):
                                if download_attempt < download_max_attempt:
                                    ...
                                    vb.write(
                                        verbose=True,
                                        content=f"\n-----> Worker: {worker.id} - Attempt: [{worker.attempt}/{retry_max_attempt}] Snapshot ID: [{worker.counter}/{self.sc._snapshot_total}] - {e.__class__.__name__} - requesting again in 50 seconds...",
                                    )
                                    vb.write(
                                        verbose=False,
                                        content=f"Worker: {worker.id} - Snapshot {worker.counter}/{self.sc._snapshot_total} - requesting again in 50 seconds...",
                                    )
                                    time.sleep(50)
                                    continue
                                ...
                                if download_attempt < download_max_attempt:
                                    ...
                                    vb.write(
                                        verbose=True,
                                        content=f"\n-----> Worker: {worker.id} - Attempt: [{worker.attempt}/{retry_max_attempt}] Snapshot ID: [{worker.counter}/{self.sc._snapshot_total}] - {e.__class__.__name__} - renewing connection in 15 seconds...",
                                    )
                                    vb.write(
                                        verbose=False,
                                        content=f"Worker: {worker.id} - Snapshot {worker.counter}/{self.sc._snapshot_total} - renewing connection in 15 seconds...",
                                    )
                                    time.sleep(15)
                                ...
                                ex.exception(
                                    f"\n-----> Worker: {worker.id} - Attempt: [{worker.attempt}/{retry_max_attempt}] Snapshot ID: [{worker.counter}/{self.sc._snapshot_total}] - EXCEPTION - {e}",
                                    e=e,
                                )
...
def download(output, no_redirect=False, worker=None):
    ...
    if not no_redirect and response_status == 302:
        ...
        for _ in range(5):
            response, response_data, response_status, response_status_message = download_response(
