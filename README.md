...
- `--skip`: Checks the skipping-file (`waybackup_<domain>.skip`) in the output-dir or a specified path and skips all files which are already downloaded. If there is no success-log, it will be created. This is useful for interrupted downloads. Files will be checked by their root-domain. So if you run a query like `example.com/subdir1/subdir2` - keep the logfile and later run a query for `example.com` the same logfile will be used for both queries.
